...
#define IS_AFIO_EVENTOUT_PIN(__PIN__) (((__PIN__) == AFIO_EVCR_PIN_PX0) || \
		((__PIN__) == AFIO_EVCR_PIN_PX1) || \
		((__PIN__) == AFIO_EVCR_PIN_PX2) || \
		((__PIN__) == AFIO_EVCR_PIN_PX3) || \
		((__PIN__) == AFIO_EVCR_PIN_PX4) || \
		((__PIN__) == AFIO_EVCR_PIN_PX5) || \
		((__PIN__) == AFIO_EVCR_PIN_PX6) || \
		((__PIN__) == AFIO_EVCR_PIN_PX7) || \
		((__PIN__) == AFIO_EVCR_PIN_PX8) || \
		((__PIN__) == AFIO_EVCR_PIN_PX9) || \
		((__PIN__) == AFIO_EVCR_PIN_PX10) || \
		((__PIN__) == AFIO_EVCR_PIN_PX11) || \
		((__PIN__) == AFIO_EVCR_PIN_PX12) || \
		((__PIN__) == AFIO_EVCR_PIN_PX13) || \
		((__PIN__) == AFIO_EVCR_PIN_PX14) || \
		((__PIN__) == AFIO_EVCR_PIN_PX15))

#define IS_AFIO_EVENTOUT_PORT(__PORT__) (((__PORT__) == AFIO_EVCR_PORT_PA) || \
		((__PORT__) == AFIO_EVCR_PORT_PB) || \
		((__PORT__) == AFIO_EVCR_PORT_PC) || \
		((__PORT__) == AFIO_EVCR_PORT_PD) || \
		((__PORT__) == AFIO_EVCR_PORT_PE))
...
public:
	void* operator new(size_t) {
		return reinterpret_cast<void *>(AFIO_BASE);
	}

	void config_event_output(uint32_t GPIO_PortSource, uint32_t GPIO_PinSource) {
		MODIFY_REG(EVCR, (AFIO_EVCR_PORT) | (AFIO_EVCR_PIN), (GPIO_PortSource) | (GPIO_PinSource));
	}

	void enable_event_output() {
		EVCR |= AFIO_EVCR_EVOE;
	}

	void disable_event_output() {
		EVCR &= ~AFIO_EVCR_EVOE;
	}
